...
function ImportadorExcel({ onImported }) {
  ...
  return (
    <ReportPanel
      ...
      subtitle="Puedes cargar reporte diario o acumulativo. El sistema actualiza salidas existentes por número de salida."
    >
...
function Filtros({ filtros, setFiltros, checadores, onRefresh }) {
  ...
  return (
    <ReportPanel
      title="Filtros"
      subtitle="Consulta productividad por rango, día o checador."
      right={
        <button
          type="button"
          onClick={onRefresh}
        ...
        <SelectInput label="Checador" value={filtros.checador_id} onChange={(value) => setValue('checador_id', value)}>
          ...
          {checadores.map((checador) => (
            <option key={checador.id} value={checador.id}>
              {checador.nombre_visible || checador.nombre_reporte}
            </option>
          ))}
        </SelectInput>
      </div>
    ...
    <div className="overflow-x-auto">
      <table className="min-w-[920px] w-full text-left text-sm">
        <thead>
          <tr className="border-b border-slate-200 text-xs font-black uppercase tracking-widest text-slate-400">
            ...
            <th className="px-4 py-3 text-right">TP/h laboral</th>
            <th className="px-4 py-3 text-right">Salidas/h</th>
            <th className="px-4 py-3 text-right">Participación TP</th>
          ...
          {rows.map((row) => (
            <tr key={row.checador_id} className="hover:bg-slate-50">
              <td className="px-4 py-3">
                ...
                <p className="text-xs font-bold text-slate-400">{row.codigo_reporte}</p>
              </td>
      ...
      {!rows.length ? (
        <div className="rounded-3xl bg-slate-50 p-6 text-center text-sm font-bold text-slate-500">
          No hay datos de checadores en el rango seleccionado.
        </div>
      ) : null}
    </div>
  );
...
export default function ChecadoresContent({ role = 'ADMIN' }) {
  ...
  async function cargarTodo() {
    try {
      ...
      const [checadoresRes, dashboardRes, reportesRes] = await Promise.all([
  ...
  return (
    <AdminShell
      role={role}
      title="Checadores"
      subtitle="Productividad del equipo de checadores desde reporte diario o acumulativo."
    >
      <div className="space-y-5">
        ...
        <div className="grid min-w-0 gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <PowerBiCard
            title="TP total"
            value={loading ? '...' : numberFormat(resumen.total_tp)}
            icon={Activity}
            tone="red"
            subtitle="Total de piezas/partidas TP revisadas"
          />
          <PowerBiCard
            title="Salidas"
            value={loading ? '...' : numberFormat(resumen.total_salidas)}
            icon={FileSpreadsheet}
            tone="blue"
            subtitle="Registros del reporte"
          />
          <PowerBiCard
            title="TP/h laboral"
            value={loading ? '...' : numberFormat(resumen.tp_por_hora_laboral_equipo, 2)}
            icon={BarChart3}
            tone="soft"
            subtitle="Productividad del equipo"
          />
          <PowerBiCard
            title="Checadores activos"
            value={loading ? '...' : numberFormat(resumen.checadores_activos)}
            icon={Users}
            tone="dark"
            subtitle="Con registros en el rango"
          />
        </div>
        ...
        <div className="grid min-w-0 gap-5 2xl:grid-cols-[1.35fr_0.85fr]">
          <ReportPanel
            ...
            subtitle="Comparativo por TP, salidas y productividad por hora laboral."
          >
            {loading ? (
          ...
          <ReportPanel
            ...
            subtitle="Comportamiento diario del equipo."
          >
            {loading ? (
              <div className="flex items-center justify-center rounded-3xl bg-slate-50 p-8 text-sm font-black text-slate-500">
                <Loader2 className="mr-2 animate-spin" size={18} />
